...
def tracking(frame):
    ...
    if FLAGS.framework == 'tflite':
        ...
        if FLAGS.model == 'yolov3' and FLAGS.tiny:
            boxes, pred_conf = filter_boxes(pred[1], pred[0], score_threshold=score_threshold,
                                            input_shape=tf.constant([input_size, input_size]))
        else:
            boxes, pred_conf = filter_boxes(pred[0], pred[1], score_threshold=score_threshold,
                                            input_shape=tf.constant([input_size, input_size]))
    else:
